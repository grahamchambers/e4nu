...
void Drawplots4()
{
  ...
  TFile * output_file = new TFile("transmission_C12_2261_EMQE_G18_02a_00_000_el_mom_1.9_el_theta_20_25.root","RECREATE");
  ...
  for ( int j = 0 ; j < NEntries1 ; ++j ) 
	...
	TVector3 V3_el(pxl1[jj],pyl1[jj],pzl1[jj]);
	  if ( pdgi1[jj] == 2212 && abs(resc1[jj]) == 1 && pow(pow(El1[jj],2)-pow(Me,2),0.5) > electron_mom_cut && V3_el.Theta() * 180 / TMath::Pi() > 20 && V3_el.Theta() * 180 / TMath::Pi() < 25) //Slicing to obtain only protons that did not re-scatter and electron momentum cut
	    {
...
//		std::cout << V3_el.Theta() * 180 / TMath::Pi() << std::endl; 
