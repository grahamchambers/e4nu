...
// Users don't have to read all the code below this dashed line unless they want to do a deep dive into the code, or if they want to debug, etc.
	...
	std::string data_file_path ("/genie/app/users/nsteinbe/e4nu/CLAS/DATA/Full_Data_Sample/He4/");
	//std::string data_file_path ("/pnfs/genie/persistent/users/gchamber/output2022/Analysis14/");
	...
	std::string mc_incl[N_RANGES];
        ...
        mc_incl[0] = mc_file_path + "Inclusive/Inclusive_Range1_Genie_1_4He_2.261000.root";
        mc_incl[1] = mc_file_path + "Inclusive/Inclusive_Range2_Genie_1_4He_2.261000.root";
        mc_incl[2] = mc_file_path + "Inclusive/Inclusive_Range3_Genie_1_4He_2.261000.root";
        mc_excl[0] = mc_file_path + "Exclusive/Exclusive_Range1_Genie_1_4He_2.261000.root";
        mc_excl[1] = mc_file_path + "Exclusive/Exclusive_Range2_Genie_1_4He_2.261000.root";
        mc_excl[2] = mc_file_path + "Exclusive/Exclusive_Range3_Genie_1_4He_2.261000.root";
	...
	double MEC_corr[N_RANGES] = {0,0,0};
	double MEC_corr_err[N_RANGES] = {0,0,0};
	...
	MEC_corr[file_i] = mec[file_i]/(qel[file_i]+mec[file_i]);
	MEC_corr_err[file_i] = error2;		
	std::cout << "mec/(qel+mec), range " << file_i << ": " << mec[file_i]/(qel[file_i]+mec[file_i]) << " +/- " << error2 << std::endl;

	}


	double mc_conglomerate_trans[N_RANGES] = {0.,0.,0.}; // transparency values for each range summed up, MC
        double data_conglomerate_trans[N_RANGES] = {0.,0.,0.}; // transparency values for each range summed up, data

	double mc_conglomerate_trans_err[N_RANGES] = {0.,0.,0.}; // transparency values for each range summed up, MC
        double data_conglomerate_trans_err[N_RANGES] = {0.,0.,0.};// transparency values for each range summed up, data
	...
	for (int idx = 0; idx < N_RANGES; idx++) { 
		...
		mc_conglomerate_trans_err[idx] *= 2.; //double error for neutron correction
		mc_conglomerate_trans[idx] *= 2.; //neutron correction

		mc_conglomerate_trans_err[idx] = mc_conglomerate_trans[idx]*sqrt(pow(MEC_corr_err[idx]/MEC_corr[idx],2) + pow(mc_conglomerate_trans_err[idx]/mc_conglomerate_trans[idx],2)); //error for MEC correction
		mc_conglomerate_trans[idx] *= 1./(1. - MEC_corr[idx]); //MEC correction

		data_denom *= corrRange[idx];

		data_conglomerate_trans[idx] = data_num/data_denom;
		data_conglomerate_trans[idx] *= 1./(1. - MEC_corr[idx]);
		...
		data_conglomerate_trans_err[idx] = data_conglomerate_trans[idx]*sqrt(pow(data_conglomerate_trans_err[idx]/data_conglomerate_trans[idx],2) + pow(MEC_corr_err[idx]/MEC_corr[idx],2));	
